<script>
  ...
  function suggestedKg(index) {
    ...
    return last?.sets?.[index]?.kg ?? last?.topKg ?? null;
  }
...
<div class="overflow-hidden rounded-xl border border-white/5 bg-rack/60">
  <div class="flex w-full items-center gap-3 px-3 py-3">
    <span
      class="h-9 w-1.5 shrink-0 rounded-full"
      style="background: {task.done ? accent : 'rgba(241,238,231,0.15)'}"
    ></span>
